...
export function AssistantMessage({ message, status, isLastMessage, durations, onDurationChange }: { message: UIMessage; status?: string; isLastMessage?: boolean; durations?: Record<string, number>; onDurationChange?: (key: string, duration: number) => void }) {
    ...
    // 1. Detection Logic: Check if the message contains a Trek Plan (Markdown Table)
    // We look for typical markdown table syntax like "| Day |" or "|---"
    const isTrekPlan = message.parts.some(part => 
        part.type === 'text' && 
        (part.text.includes("| Day") || part.text.includes("| Route") || part.text.includes("|---"))
    );

    // 2. Visual Theme: Topographic Map Pattern (Subtle SVG background)
    const trekThemeClasses = isTrekPlan 
        ? "bg-emerald-50/50 border border-emerald-100 dark:bg-emerald-950/20 dark:border-emerald-900 rounded-xl p-4 shadow-sm relative overflow-hidden" 
        : "w-full";

    // This SVG pattern creates a subtle topographic line effect
    ...
    return (
        <div className={trekThemeClasses} style={topographicPattern}>
            {/* Optional Badge for Trek Plans */}
            {isTrekPlan && (
                <div className="absolute top-0 right-0 m-2 px-2 py-1 bg-emerald-100 dark:bg-emerald-900 text-emerald-700 dark:text-emerald-300 text-[10px] font-bold uppercase tracking-wider rounded-md border border-emerald-200 dark:border-emerald-800 opacity-70">
                    Trek Plan
                </div>
            )}
